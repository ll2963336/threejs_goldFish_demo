/* ES Module Shims 1.3.6 */ (function() {
    ...
    function resolveIfNotPlainOrUrl(relUrl, parentUrl) {
        ...
        if (relUrl[0] === "/" && relUrl[1] === "/") return parentUrl.slice(0, parentUrl.indexOf(":") + 1) + relUrl;
            ...
            } else // resolving to :/ so pathname is the /... part
            ...
            for(let i = 0; i < segmented.length; i++){
                ...
                if (segmentIndex !== -1) {
                    ...
                    else // the start of a new segment as below
                    segmentIndex = i;
                } else segmentIndex = i;
            }
   ...
   */ function resolveUrl(relUrl, parentUrl) {
    ...
    function resolveAndComposePackages(packages, outPackages, baseUrl, parentMap) {
        for(let p in packages){
            ...
            if (outPackages[resolvedLhs]) throw new Error(`Dynamic import map rejected: Overrides entry "${resolvedLhs}" from ${outPackages[resolvedLhs]} to ${packages[resolvedLhs]}.`);
    ...
    function applyPackages(id, packages) {
        ...
        if (pkgName) {
            ...
            if (pkg === null) return;
            if (id.length > pkgName.length && pkg[pkg.length - 1] !== "/") targetWarning(pkgName, pkg, "should have a trailing '/'");
    ...
    function parse(k, l = "@") {
        if (t = k, f = l, t.length > i || !e) {
            ...
            r = new ArrayBuffer(4 * i), e = function(e, r, a) {
                "use asm";
                var i = new e.Int8Array(a), s = new e.Int16Array(a), t = new e.Int32Array(a), f = new e.Uint8Array(a), c = new e.Uint16Array(a), n = 816;
                function b(e) {
                    e = e | 0;
                    var r = 0, a = 0, f = 0, b = 0, l = 0;
                    l = n;
                    n = n + 14336 | 0;
                    b = l;
                    i[589] = 1;
                        ...
                        e: while(true){
                            ...
                            r: do switch(s[r >> 1] | 0){
                                ...
                                case 123:
                                    ...
                                    a = t[8] | 0;
                                    e = f;
                                    do if ((s[f >> 1] | 0) == 41 & (a | 0) != 0 ? (t[a + 4 >> 2] | 0) == (f | 0) : 0) {
                                        r = t[9] | 0;
                                        t[8] = r;
                                        if (!r) {
                                            t[4] = 0;
                ...
                function k() {
                    var e = 0, r = 0, a = 0, f = 0, c = 0;
                    c = t[18] | 0;
                    r = c + 12 | 0;
                    t[18] = r;
                    e: do switch((w(1) | 0) << 16 >> 16){
                        case 40:
                            r = t[17] | 0;
                            a = s[292] | 0;
                            s[292] = a + 1 << 16 >> 16;
        ...
        const w = [], d = [];
        for(; e.ri();){
            const r1 = e.is(), a1 = e.ie(), i1 = e.ai(), s1 = e.id(), f1 = e.ss(), c = e.se();
            let n1;
            e.ip() && (n1 = u(-1 === s1 ? r1 : r1 + 1, t.charCodeAt(-1 === s1 ? r1 - 1 : r1))), w.push({
                n: n1,
                s: r1,
                e: a1,
                ss: f1,
        ...
        for(; e.re();){
            ...
            d.push(34 === a2 || 39 === a2 ? u(r2 + 1, a2) : t.slice(e.es(), e.ee()));
        }
        return [
            w,
            d,
    ...
    function u(e, r) {
        ...
        for(;;){
            c$1 >= t.length && h();
            const e1 = t.charCodeAt(c$1);
            if (e1 === r) break;
            92 === e1 ? (a += t.slice(i, c$1), a += k(), i = c$1) : (8232 === e1 || 8233 === e1 || o(e1) && h(), ++c$1);
        }
    ...
    function k() {
        ...
        switch(++c$1, e){
            ...
            default:
                if (e >= 48 && e <= 55) {
                    let r = t.substr(c$1 - 1, 3).match(/^[0-7]+/)[0], a = parseInt(r, 8);
    ...
    function l(e) {
        const r = c$1;
        let a = 0, i = 0;
        for(let r1 = 0; r1 < e; ++r1, ++c$1){
            let e1, s = t.charCodeAt(c$1);
            if (95 !== s) {
                if (s >= 97) e1 = s - 97 + 10;
                else if (s >= 65) e1 = s - 65 + 10;
                else {
                    if (!(s >= 48 && s <= 57)) break;
                    e1 = s - 48;
                }
                ...
                for (const mutation of mutations){
                    ...
                    for (const node of mutation.addedNodes){
                        if (node.tagName === "SCRIPT") {
                            if (!shimMode && node.type === "module" || shimMode && node.type === "module-shim") processScript(node);
                            if (!shimMode && node.type === "importmap" || shimMode && node.type === "importmap-shim") processImportMap(node);
                        } else if (node.tagName === "LINK" && node.rel === "modulepreload") processPreload(node);
                    }
                }
            }).observe(document, {
                childList: true,
    ...
    async function topLevelLoad(url, fetchOpts, source, nativelyLoaded, lastStaticLoadPromise) {
        ...
        const module1 = await dynamicImport(!shimMode && !load.n && nativelyLoaded ? load.u : load.b, {
            errUrl: load.u
        });
        // if the top-level load is a shell, run its update function
        if (load.s) (await dynamicImport(load.s)).u$_(module1);
        if (revokeBlobURLs) revokeObjectURLs(Object.keys(seen));
    ...
    async function importShim(id, parentUrl = baseUrl, _assertion) {
        ...
        await importMapPromise;
        return topLevelLoad((await resolve(id, parentUrl)).r || throwUnresolved(id, parentUrl), {
            credentials: "same-origin"
        });
    }
    self.importShim = importShim;
    if (shimMode) importShim.getImportMap = ()=>JSON.parse(JSON.stringify(importMap));
    const meta = {};
    ...
    function resolveDeps(load, seen) {
        ...
        if (!imports.length) resolvedSource += source;
        else {
            ...
            for (const { s: start , se: end , d: dynamicImportIndex  } of imports){
                ...
                if (dynamicImportIndex === -1) {
                    ...
                    if (!blobUrl) // circular shell creation
                    {
                        if (!(blobUrl = depLoad.s)) blobUrl = depLoad.s = createBlob(`export function u$_(m){${depLoad.a[1].map((name)=>name === "default" ? `$_default=m.default` : `${name}=m.${name}`).join(",")}}${depLoad.a[1].map((name)=>name === "default" ? `let $_default;export{$_default as default}` : `export let ${name}`).join(";")}\n//# sourceURL=${depLoad.r}?cycle`);
                    } else if (depLoad.s) {
    ...
    async function doFetch(url, fetchOpts) {
        ...
        else if (cssContentType.test(contentType)) return {
            r: res.url,
        ...
        else if (wasmContentType.test(contentType)) throw new Error("WASM modules not supported");
    ...
    function getOrCreateLoad(url, fetchOpts, source) {
        ...
        load.f = (async ()=>{
            if (!source) {
                ...
                if (t && !shimMode) {
                    if (t === "css" && !cssModulesEnabled || t === "json" && !jsonModulesEnabled) throw new Error(`${t}-modules require <script type="esms-options">{ "polyfillEnable": ["${t}-modules"] }<${""}/script>`);
    ...
    else document.addEventListener("readystatechange", async ()=>{
        processImportMaps();
        await initPromise;
...
//# sourceMappingURL=index.1224ad90.js.map
